...
toastr.options = {
    ...
    $('#send').click(function () {
        ...
        $.ajax({
            type: "GET",
            url: "{{ url('send-patient-condition') }}",
            ...
            success: function (response) {
                 ...
                 if (response.error.length > 0) {
                    ...
                    response.error.forEach(error => {
                        toastr.error('Patient Condition Send Failed', 'Error');
                    });
                } else {
                       const successCount = response.success.length;
                    // Display a success Toastr alert with the count
                    toastr.success(`${successCount} patient(s) Condition Sent successfully`, 'Success');
