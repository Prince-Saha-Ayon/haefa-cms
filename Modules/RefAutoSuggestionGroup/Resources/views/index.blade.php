...
@include('refautosuggestiongroup::modal')
{{-- @include('refautosuggestiongroup::add-edit-modal') --}}
@endsection
...
$(document).ready(function(){
    ...
    table = $('#dataTable').DataTable({
        ...
        "ajax": {
            "url": "{{route('refautosuggestiongroup.datatable.data')}}",
            "type": "POST",
            "data": function (data) {
        ...
        "buttons": [
            @if (permission('refautosuggestiongroup-report'))
            {
    ...
    $(document).on('click', '#save-btn', function () {
        ...
        let url = "{{route('refautosuggestiongroup.store.or.update')}}";
    ...
    $(document).on('click', '.view_data', function () {
       ...
       // let date = $(this).data('date');
        if (id) {
            $.ajax({
                url: "{{route('refautosuggestiongroup.show')}}",
                type: "POST",
    ...
    $(document).on('click', '.delete_data', function () {
        ...
        let name  = "refautosuggestiongroup";
        console.log(name);
        let row   = table.row($(this).parent('tr'));
        let url   = "{{ route('refautosuggestiongroup.delete') }}";
    ...
    function multi_delete(){
        ...
        if(ids.length == 0){
            ...
            let url = "{{route('refautosuggestiongroup.bulk.delete')}}";
...
$(document).on('click', '.change_status', function () {
    ...
    let name  = $(this).data('name');
    let row   = table.row($(this).parent('tr'));
    let url   = "{{ route('refautosuggestiongroup.change.status') }}";
    Swal.fire({
...
$(document).on('click', '.edit_data', function () {
    ...
    if (id) {
        $.ajax({
            url: "{{route('refautosuggestiongroup.edit')}}",
            type: "POST",
            data: { id: id,_token: _token},
            dataType: "JSON",
            success: function (data) {
                ...
                $('#RefAutoSuggestionGroupCode').val(data.RefAutoSuggestionGroupCode);
